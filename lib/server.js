...
function handle(request,response){
    var isApi = false,tempData = '';

    var host = request.headers.host.replace(/:\d*/gi,'');

    if(request.url != '/' && Config.api){
        Config.api.forEach(function(data){
            if(request.url.indexOf(data.src) > -1 && !tempData){
                isApi = true;
                tempData = data;
            }
        });
    ...
    if(isApi){
        ...
        var contentType = tempData.mime ? mime[tempData.mime] : "text/plain";
        response.writeHead(200, {
            'Content-Type': contentType+';charset:utf-8',
        });
        response.write(code.toString());
        response.end();
    }else if(request.url == '/_/api/update'){
        var postdata = '';
            ...
            if(fs.existsSync(source)){
                fs.readFile(source,'binary',function(err,code){
                    ...
                    if(contentType === "text/html"){
                        code = code.replace(/<\/body>/gi, "<script src='http://"+host+":35729/livereload.js'></script>\n</body>");
                    };
                    response.writeHead(200, {
